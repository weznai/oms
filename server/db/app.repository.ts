  ...
  const res = await getDb().run(
    `INSERT INTO sys_app
     (name, display_name, type, scope, repo_url, branch, deploy_path, pm2_app_name, port,
      install_cmd, build_cmd, build_enabled, start_file, interpreter, deploy_excludes, enabled, remark, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      ...
      data.build_enabled ? 1 : 0, data.start_file, data.interpreter, data.deploy_excludes,
...
export async function updateApp(id: number, data: AppInput): Promise<void> {
  ...
  await getDb().run(
    `UPDATE sys_app SET
       name=?, display_name=?, type=?, scope=?, repo_url=?, branch=?, deploy_path=?, pm2_app_name=?, port=?,
       install_cmd=?, build_cmd=?, build_enabled=?, start_file=?, interpreter=?, deploy_excludes=?, enabled=?, remark=?, updated_at=?
     WHERE id=?`,
    [
      data.name, data.display_name, data.type, data.scope, data.repo_url, data.branch,
      data.deploy_path, data.pm2_app_name, data.port, data.install_cmd, data.build_cmd,
