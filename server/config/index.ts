...
export const config = {
  ...
  jwtSecret: env('JWT_SECRET', 'oss-ops-dev-secret'),
  admin: {
  ...
  db: {
    ...
    sqlite: {
      path: path.resolve(projectRoot, env('DB_SQLITE_PATH', './db/oss-ops.db'))
    },
    mysql: {
      ...
      database: env('DB_MYSQL_DATABASE', 'oss_ops'),
  ...
  update: {
    ...
    pm2AppName: env('UPDATE_PM2_APP_NAME', 'oss-ops'),
